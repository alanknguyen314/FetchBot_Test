...
def calculate_floor_projection(initial_position, velocity):
    ...
    if z0 <= 0:
        return []  # No motion if the object is already on the ground
...
def visualize_3d_trajectory_with_projection(trajectory, initial_position, velocity, time_to_ground):
    ...
    # Automatically scale axes based on trajectory data
    ax.set_xlim(0, max(trajectory[:, 0]) * 1.2)
    ax.set_ylim(0, max(trajectory[:, 1]) * 1.2)
    ax.set_zlim(0, max(trajectory[:, 2]) * 1.2)
