...
def visualize_3d_trajectory(trajectory, initial_position, velocity, time_to_ground):
    """
    Visualizes the predicted 3D trajectory with additional details.
    ...
    ax.text(initial_position[0], initial_position[1], initial_position[2], f"Velocity: {velocity:.2f} m/s\nTime to Ground: {time_to_ground:.2f}s",
    ...
    # 2D Projection on the XY plane (Floor)
    ax2 = fig.add_subplot(122)
    ax2.plot(trajectory[:, 0], trajectory[:, 1], label="2D Floor Projection", color='g')
    ...
    try:
        while True:
            ...
            if not prediction_made and current_time - prev_time > 2.0:  # After 2 seconds
                ...
                if trajectory:
                    # Calculate velocity and time to ground
                    x1, y1, z1 = trajectory[0]
                    x2, y2, z2 = trajectory[1]
                    velocity = np.sqrt((x2 - x1)**2 + (y2 - y1)**2 + (z2 - z1)**2) / 0.1  # Assuming 0.1s time step
                    time_to_ground = np.sqrt(2 * initial_position[2] / 9.81)  # Free-fall time calculation

                    # Visualize trajectory
                    visualize_3d_trajectory(trajectory, initial_position, velocity, time_to_ground)
